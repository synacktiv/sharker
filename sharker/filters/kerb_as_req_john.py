...
class FilterConfig(FilterConfigBase):
    ...
    def gen_hash(self, etype, client_name, service_name, packet_hash, salt, domain):
        ...
        if etype == "23":
            ...
            if salt is None:
                self.log.error('Got a Kerberos pre-authentication (other than RC4) without prior salt given by the KDC. We have to manually compute salt.')
                if '.' not in domain:
